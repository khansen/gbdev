...
def SOUND_STATUS_PAUSED equ 4
def SOUND_STATUS_PENDING_AUDTERM equ 5
...
EXPORT UnpauseMusic
endc
EXPORT GetMutedChannelsMask
EXPORT SetMutedChannelsMask
if def(SOUND_ENGINE_ENABLE_STOP)
EXPORT StopSound
endc
if def(SOUND_ENGINE_ENABLE_PAUSE)
EXPORT QueueSoundOutputMask
EXPORT SilenceSoundOutput
EXPORT RestoreSoundOutput
endc
if def(SOUND_ENGINE_ENABLE_SFX)
EXPORT PlayTrack0SFX
EXPORT PlayTrack1SFX
EXPORT PlayTrack2SFX
EXPORT PlayTrack3SFX
endc
if def(SOUND_ENGINE_ENABLE_PATTERN_ROW_CALLBACK)
EXPORT SetPatternRowCallback
endc
if def(SOUND_ENGINE_ENABLE_PREROLL)
EXPORT hSoundPrerollRowsRemaining
endc
EXPORT hShadowNR12
EXPORT hShadowNR22
EXPORT hShadowNR32
...
SECTION "Sound WRAM", WRAM0, ALIGN[8]
...
; Extracted from games/d-pad-hero/main.s.
...
GetMutedChannelsMask:
    ldh a, [hSoundStatus]
    and %00001111
    ret

; A = channel mute bit mask
SetMutedChannelsMask:
    and %00001111
    ld b, a
    ldh a, [hSoundStatus]
    and %11110000
...
if def(SOUND_ENGINE_ENABLE_PAUSE)
; A = NR51 output routing mask to apply after the next UpdateSound render.
; The normal shadow is preserved, so pause/resume routing is unaffected.
QueueSoundOutputMask:
    ldh [hPendingAUDTERM], a
    ldh a, [hSoundStatus]
    set SOUND_STATUS_PENDING_AUDTERM, a
    ldh [hSoundStatus], a
    ret
...
SilenceSoundOutput:
    ...
    ldh [rAUDTERM], a
    ldh a, [hSoundStatus]
    res SOUND_STATUS_PENDING_AUDTERM, a
    ldh [hSoundStatus], a
    ret
...
endc
    ...
    call RenderChannel4
if def(SOUND_ENGINE_ENABLE_PAUSE)
    ; Apply pending sound output mask, if any
    ldh a, [hSoundStatus]
    bit SOUND_STATUS_PENDING_AUDTERM, a
    ret z
    res SOUND_STATUS_PENDING_AUDTERM, a
    ldh [hSoundStatus], a
    ldh a, [hPendingAUDTERM]
    ldh [rAUDTERM], a
endc
    ret
...
if def(SOUND_ENGINE_ENABLE_PAUSE)
    ldh a, [hSoundStatus]
    bit SOUND_STATUS_PAUSED, a
    jr nz, .skip_audterm_write
    ldh a, [hShadowAUDTERM]
    ldh [rAUDTERM], a
    .skip_audterm_write:
endc
...
SECTION "Sound Engine Tables", ROM0, ALIGN[8]
