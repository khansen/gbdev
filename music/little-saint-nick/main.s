...
update_graphics_ram: .space (update_graphics_rom_end - update_graphics_rom)
...
_start:
    ...
    bl copy_update_graphics_from_rom_to_ram
...
copy_update_graphics_from_rom_to_ram:
    ldr r0, =update_graphics_rom
    ldr r1, =update_graphics_ram
    mov r2, #((update_graphics_rom_end - update_graphics_rom) / 4)
    orr r2, r2, #(DMA_ENABLE | DMA_START_NOW | DMA_32BIT | DMA_SRC_INC | DMA_DST_INC)
    b dma3

call_update_graphics:
    push {lr}
    adr lr, 1f
    ldr r0, =update_graphics_ram
    bx r0
    1:
    pop {lr}
    bx lr
...
play_note_callback:
    ...
    cmp r0, #FIRE_COLUMNS
    bcc 1f
    sub r0, r0, #FIRE_COLUMNS
    b 1b
    1:
    ldr r2, =(fire_data+(1+FIRE_COLUMNS+1)*(FIRE_ROWS-6))
    add r2, r2, r0
    mov r3, #255
    strb r3, [r2, #1]
    strb r3, [r2, #2]
    strb r3, [r2, #(1+FIRE_COLUMNS+1)]
    strb r3, [r2, #(1+FIRE_COLUMNS+1+1)]
    strb r3, [r2, #(1+FIRE_COLUMNS+1+2)]
    strb r3, [r2, #(1+FIRE_COLUMNS+1+3)]
    strb r3, [r2, #((1+FIRE_COLUMNS+1)*2)]
    strb r3, [r2, #((1+FIRE_COLUMNS+1)*2+1)]
    strb r3, [r2, #((1+FIRE_COLUMNS+1)*2+2)]
    strb r3, [r2, #((1+FIRE_COLUMNS+1)*2+3)]
    strb r3, [r2, #((1+FIRE_COLUMNS+1)*3+1)]
    strb r3, [r2, #((1+FIRE_COLUMNS+1)*3+2)]
    pop {r0, r1}
    bx lr
...
update_graphics_rom:
    ...
    mov r3, #FIRE_COLUMNS
    2:
    ldrb r4, [r0, #(1+FIRE_COLUMNS+1)] @ one row below
    ldrb r5, [r0, #((1+FIRE_COLUMNS+1)*2-1)] @ two rows below and to the left
    ldrb r6, [r0, #(1+FIRE_COLUMNS+1)*2] @ two rows below
    ldrb r7, [r0, #((1+FIRE_COLUMNS+1)*2+1)] @ two rows below and to the right
    lsl r4, r4, #4
    lsl r6, r6, #4
    add r4, r4, r6
    lsr r6, r6, #2
    ...
    subs r3, r3, #1
    bne 2b
    add r0, r0, #2 @ column 1 of next row
    subs r2, r2, #1
    bne 1b

    @ generate row of random values
    mov r4, r0
    mov r3, #(FIRE_COLUMNS+1)
    1:
    generate_random_byte
    strb r0, [r4], #1
    subs r3, r3, #1
...
.if 0
    @ generate row of max intensity values
    mov r0, #255
    mov r3, #(1+FIRE_COLUMNS+1)
    1:
    strb r0, [r4], #1
    subs r3, r3, #1
    bne 1b
.endif
    ...
    @ translate fire data to framebuffer
    ldr r0, =current_framebuffer
    ldrb r0, [r0]
    tst r0, r0
    ldr r0, =VIDEO_BUFFER
    addne r0, r0, #0xA000 @ target buffer
    ldr r2, =((160*65+60)*2) @ start offset in buffer
    add r0, r0, r2
    ldr r1, =(fire_data+1)
    ldr r5, =palette
    mov r2, #(FIRE_ROWS-1)
    ...
    subs r3, r3, #1
    bne 2b
    add r0, r0, #((160-FIRE_COLUMNS)*2) @ advance to start of next row
    add r1, r1, #2 @ column 1 of next row
    subs r2, r2, #1
    bne 1b
    bx lr
.ltorg
update_graphics_rom_end:
