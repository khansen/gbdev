...
update_graphics_rom:
    ...
    mov r4, r0
    mov r5, #4 @ number of candles (channels)
    1: @ for each candle
    mov r3, #CANDLE_FLAME_WIDTH
    2:
    generate_random_byte
    strb r0, [r4], #1
    mov r0, #255
    strb r0, [r4, #(1+FIRE_COLUMNS+1)] @ one row below
    subs r3, r3, #1
    bne 2b
    add r4, r4, #CANDLE_FLAME_SPACE
    subs r5, r5, #1
    bne 1b
    ...
    ldr r1, =(fire_data+1)
    ldr r5, =palette
    eor r6, r6, r6 @ candle index
    1: @ for each candle
    push {r0, r1}
    ldr r2, =candle_framebuffer_offsets
    ldr r2, [r2, r6, lsl #2]
    add r0, r0, r2
    mov r2, #(FIRE_ROWS-1)
    2:
    ...
    subs r3, r3, #1
    bne 3b
    add r0, r0, #((160 - CANDLE_FLAME_WIDTH) * 2) @ start of next row in framebuffer
    add r1, r1, #(FIRE_COLUMNS + 2 - CANDLE_FLAME_WIDTH) @ start of next row in fire data
    subs r2, r2, #1
    bne 2b
    pop {r0, r1}
    add r1, r1, #(CANDLE_FLAME_WIDTH + CANDLE_FLAME_SPACE)
    add r6, r6, #1 @ next candle index
    cmp r6, #4 @ done all candles?
    bne 1b
    bx lr
.ltorg
